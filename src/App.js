...
const TARJETAS = [
  { key:'ob',       label:'🚫 Out of Bound' },
  { key:'water',    label:'💧 Water'         },
  { key:'sand',     label:'⛱️ Sand'          },
  { key:'sapo',     label:'🐸 Sapo'          },
  { key:'arbol',    label:'🌳 Árbol'         },
  { key:'threeput', label:'🔄 Three putt'    },
];
 
/* ─── DISEÑO ─────────────────────────────────────── */
const D = {
  bg:'#0A0A0A', surface:'#141414', card:'#1A1A1A', border:'#2A2A2A',
  gold:'#C9A84C', goldLight:'#E8C97A', goldDim:'#6B5520',
  text:'#F5F5F5', textSub:'#888888', textDim:'#444444',
  green:'#2E7D32', greenBg:'#0D2B0F', red:'#C62828', redBg:'#2B0D0D',
  success:'#4CAF50', danger:'#EF5350',
};
 
const COLORS = [
  {bg:'#1A2744',fg:'#7BAEE8'},{bg:'#1A3020',fg:'#7EC68A'},
  {bg:'#3A2010',fg:'#E8A06A'},{bg:'#2A1020',fg:'#E87AAA'},
  {bg:'#201A3A',fg:'#9A8AE8'},{bg:'#301830',fg:'#C87AC8'},
  {bg:'#102820',fg:'#6AC8A0'},{bg:'#302010',fg:'#C8A050'},
  {bg:'#181830',fg:'#8888E8'},{bg:'#301820',fg:'#E88898'},
];
...
const Avatar = ({ name, id, size=32 }) => {
  const c = col(id);
  return <div style={{ width:size, height:size, borderRadius:'50%', background:c.bg, color:c.fg, display:'flex', alignItems:'center', justifyContent:'center', fontSize:size*0.34, fontWeight:700, flexShrink:0, border:`1px solid ${c.fg}33` }}>{name.substring(0,2).toUpperCase()}</div>;
};
 
const BADGE = (s,par) => {
  const d=s-par;
  if(d<=-2) return{label:'Eagle', bg:'#1A2744',fg:'#7BAEE8'};
  if(d===-1) return{label:'Birdie',bg:'#1A3020',fg:'#7EC68A'};
  if(d===0)  return{label:'Par',  bg:'#222',   fg:'#888'};
  if(d===1)  return{label:'Bogey',bg:'#2A1A0A',fg:'#C8904A'};
  if(d===2)  return{label:'Doble',bg:'#2A1010',fg:'#E87070'};
  return{label:'+'+d,bg:'#2A0A0A',fg:'#EF5350'};
};
 
const emptyMarca    = (n) => ({ multi:Array(n).fill(null).map(()=>({holeinone:false,eagle:false,birdie:false,holeout:false,sandy:false})), oyes:null, regulation:null });
const emptyTarjetas = ()  => { const t={}; TARJETAS.forEach(tj=>{t[tj.key]=null;}); return t; };
 
const calcMarcasPts = (players, marcas) => players.map((_,pi)=>{
  let pts=0;
  marcas.forEach(h=>{ MARCAS_MULTI.forEach(m=>{if(h.multi[pi][m.key])pts+=m.pts;}); if(h.oyes===pi)pts+=1; if(h.regulation===pi)pts+=1; });
  return pts;
});
 
const calcMarcasMoney = (players, marcas) => {
  const pts=calcMarcasPts(players,marcas);
  return players.map((_,i)=>{ let b=0; players.forEach((_,j)=>{if(i!==j){b+=pts[i]*MARCA_VAL;b-=pts[j]*MARCA_VAL;}}); return b; });
};
 
const calcTarjetasMoney = (players, tarjetas) => {
  const count=players.map((_,i)=>TARJETAS.filter(t=>tarjetas[t.key]===i).length);
  return players.map((_,i)=>{ let b=0; players.forEach((_,j)=>{if(i!==j){b-=count[i]*TARJETA_VAL;b+=count[j]*TARJETA_VAL;}}); return b; });
};
 
const calcMoney = (players, scores) => {
  ...
  return { nets, fi, si, pot, money: players.map((_,i)=>{
    if(n<=9){ if(fi.includes(i)) return Math.round(totalFromLosers/fi.length); return -APUESTA; }
    if(fi.length>1){ if(fi.includes(i)) return Math.round(totalFromLosers/fi.length); return -APUESTA; }
...
const calcHC = (players, scores) => {
  const nets=players.map((p,i)=>scores[i].reduce((a,b)=>a+b,0)-p.hc);
  const fnet=Math.min(...nets);
  const fi=nets.reduce((a,v,i)=>v===fnet?[...a,i]:a,[]);
  const deltas={};
  players.forEach(p=>{deltas[p.id]=0;});
  fi.forEach(wi=>{ const w=players[wi]; if(w.hc===0) players.forEach((p,i)=>{if(i!==wi)deltas[p.id]+=1;}); else deltas[w.id]-=1; });
  return players.map(p=>({...p,before:p.hc,delta:deltas[p.id],after:Math.max(0,p.hc+deltas[p.id])}));
};
 
/* ─── UI COMPONENTS ──────────────────────────────── */
const Card = ({children,style={}}) => <div style={{background:D.card,border:`1px solid ${D.border}`,borderRadius:16,padding:'16px',marginBottom:12,...style}}>{children}</div>;
const SLabel = ({children}) => <div style={{fontSize:10,fontWeight:700,color:D.gold,textTransform:'uppercase',letterSpacing:2,marginBottom:10}}>{children}</div>;
const Btn = ({children,onClick,disabled,outline,danger,style={}}) => (
  <button onClick={onClick} disabled={disabled} style={{width:'100%',padding:14,border:outline?`1px solid ${danger?D.danger:D.gold}`:'none',borderRadius:12,fontSize:15,fontWeight:700,cursor:disabled?'default':'pointer',marginTop:6,background:outline?'transparent':danger?D.danger:`linear-gradient(135deg,${D.gold},${D.goldLight})`,color:outline?(danger?D.danger:D.gold):'#0A0A0A',opacity:disabled?0.4:1,...style}}>{children}</button>
);
const TabBar = ({tabs,active,onChange}) => (
  <div style={{display:'flex',gap:6,marginBottom:14}}>
    {tabs.map(t=><button key={t.key} onClick={()=>onChange(t.key)} style={{flex:1,padding:'9px 4px',border:`1px solid ${active===t.key?D.gold:D.border}`,borderRadius:10,background:active===t.key?D.goldDim:D.surface,color:active===t.key?D.gold:D.textSub,fontSize:11,fontWeight:700,cursor:'pointer'}}>{t.label}</button>)}
  </div>
);
const Pill = ({active,danger,onClick,children}) => (
  <div onClick={onClick} style={{display:'flex',alignItems:'center',gap:6,padding:'7px 14px',border:`1px solid ${active?D.gold:danger?D.danger:D.border}`,borderRadius:20,background:active?D.goldDim:'transparent',color:active?D.gold:danger?D.danger:D.textSub,fontSize:13,fontWeight:600,cursor:'pointer',userSelect:'none'}}>{children}</div>
);
 
/* ═══════════════════════════════════════════════════
   PANTALLA ESPECTADOR — solo lectura en tiempo real
═══════════════════════════════════════════════════ */
function SpectatorView({ rondaId }) {
  ...
  useEffect(() => {
    const rondaRef = ref(db, `rondas/${rondaId}`);
    const unsub = onValue(rondaRef, snap => {
      if (snap.exists()) { setRonda(snap.val()); setLoading(false); }
      else { setLoading(false); }
    });
    return () => unsub();
  }, [rondaId]);
 
  const app = { fontSize:14, fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif', color:D.text, background:D.bg, minHeight:'100vh', maxWidth:420, margin:'0 auto' };
 
  if (loading) return <div style={{...app,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:12}}><div style={{fontSize:32}}>⛳</div><div style={{color:D.gold,fontWeight:700}}>Conectando...</div></div>;
  if (!ronda)  return <div style={{...app,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:12}}><div style={{fontSize:32}}>🏌️</div><div style={{color:D.textSub}}>Ronda no encontrada</div></div>;
 
  const { players, scores, pars, hole, campo, tarjetas, marcas, status } = ronda;
  if (!players || !scores) return null;
  ...
  return (
    <div style={app}>
      ...
      <div style={{background:D.surface,borderBottom:`1px solid ${D.border}`,padding:'20px 16px 14px',textAlign:'center'}}>
        <div style={{fontSize:30,fontWeight:900,letterSpacing:-1,background:`linear-gradient(135deg,${D.gold},${D.goldLight})`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>H19</div>
        ...
        <div style={{marginTop:8,display:'inline-flex',alignItems:'center',gap:6,padding:'4px 12px',background:status==='finalizada'?D.greenBg:D.goldDim,border:`1px solid ${status==='finalizada'?D.success:D.gold}`,borderRadius:20}}>
          ...
          <span style={{fontSize:11,fontWeight:700,color:status==='finalizada'?D.success:D.gold}}>{status==='finalizada'?'Ronda finalizada':'En vivo'}</span>
        </div>
      </div>
      ...
      <div style={{padding:'12px 12px 32px'}}>
        ...
        <Card>
          ...
          {ranked.map((p,pos)=>{
            const holeScores = scores[players.findIndex(pl=>pl.id===p.id)]||[];
            const currentScore = holeScores[hole||0];
            const par = (pars||[])[hole||0];
            const b = currentScore && par ? BADGE(currentScore, par) : null;
            return (
        ...
        {/* Scorecard tabla */}
        <Card>
          <SLabel>📋 Scorecard</SLabel>
          <div style={{overflowX:'auto'}}>
            <table style={{borderCollapse:'collapse',width:'100%',fontSize:11,color:D.text}}>
              <thead>
                <tr style={{background:D.surface}}>
                  <td style={{padding:'6px 8px',fontWeight:700,color:D.gold,fontSize:10,position:'sticky',left:0,background:D.surface,borderBottom:`1px solid ${D.border}`,minWidth:70}}>Jugador</td>
                  {(pars||[]).map((_,i)=><td key={i} style={{padding:'5px 3px',textAlign:'center',fontWeight:700,color:D.textSub,borderBottom:`1px solid ${D.border}`,minWidth:26,fontSize:10}}>{i+1}</td>)}
                  <td style={{padding:'5px 6px',textAlign:'center',fontWeight:700,color:D.gold,borderBottom:`1px solid ${D.border}`,minWidth:34}}>NET</td>
                </tr>
                <tr>
                  <td style={{padding:'3px 8px',fontSize:10,color:D.textDim,position:'sticky',left:0,background:D.card,borderBottom:`1px solid ${D.border}`}}>PAR</td>
                  {(pars||[]).map((p,i)=><td key={i} style={{padding:'3px 3px',textAlign:'center',fontSize:10,color:D.textSub,borderBottom:`1px solid ${D.border}`}}>{p}</td>)}
                  <td style={{padding:'3px 6px',textAlign:'center',fontSize:10,color:D.textSub,borderBottom:`1px solid ${D.border}`}}>{(pars||[]).reduce((a,b)=>a+b,0)}</td>
                </tr>
              </thead>
              <tbody>
                {ranked.map((p,pos)=>{
                  ...
                  return (
                    <tr key={p.id} style={{borderBottom:`1px solid ${D.border}`,background:pos===0?'#1A1200':D.card}}>
                      <td style={{padding:'6px 8px',position:'sticky',left:0,background:pos===0?'#1A1200':D.card,zIndex:1}}>
                        <div style={{display:'flex',alignItems:'center',gap:5}}>
                          <span style={{fontSize:10,fontWeight:700,color:pos===0?D.gold:D.textSub}}>{pos+1}</span>
                          <Avatar name={p.name} id={p.id} size={18}/>
                          <span style={{fontSize:10,fontWeight:600,whiteSpace:'nowrap'}}>{p.name}</span>
                        </div>
                      </td>
                      {(pars||[]).map((par,hi)=>{
                        const s=(scores[pi]||[])[hi];
                        if(s===null||s===undefined) return <td key={hi} style={{textAlign:'center',padding:'4px 2px',color:D.textDim,fontSize:10}}>·</td>;
                        const d=s-par;
                        let cb='transparent',cc=D.text,cbr='transparent',r=2;
                        if(d<=-2){cb='#1A2744';cc='#7BAEE8';cbr='#7BAEE8';r='50%';}
                        else if(d===-1){cb='#1A3020';cc='#7EC68A';cbr='#7EC68A';r=4;}
                        else if(d===1){cc='#C8904A';cbr='#C8904A';}
                        else if(d>=2){cc=D.danger;cbr=D.danger;}
                        return <td key={hi} style={{textAlign:'center',padding:'3px 2px'}}><div style={{width:20,height:20,borderRadius:r,border:`1.5px solid ${cbr}`,background:cb,color:cc,display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontWeight:700,margin:'0 auto'}}>{s}</div></td>;
                      })}
                      <td style={{textAlign:'center',padding:'5px 6px',fontWeight:900,fontSize:12,color:D.gold}}>{p.net}</td>
                    </tr>
                  );
                })}
              </tbody>
...
export default function H19() {
  const [mode, setMode]       = useState(null); // null | 'pin' | 'admin' | 'spectator'
  ...
  if (mode === null) return <div style={{background:D.bg,minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}><div style={{color:D.gold,fontSize:20,fontWeight:700}}>Cargando H19...</div></div>;
 
  const app = { fontSize:14, fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif', color:D.text, background:D.bg, minHeight:'100vh', maxWidth:420, margin:'0 auto' };
 
  /* ── HOME ── */
  if (mode === 'home') return (
    <div style={{...app,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:24,gap:16}}>
      <div style={{fontSize:64,fontWeight:900,letterSpacing:-3,background:`linear-gradient(135deg,${D.gold},${D.goldLight})`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>H19</div>
      <div style={{fontSize:12,color:D.textSub,letterSpacing:3,textTransform:'uppercase',marginBottom:16}}>Club de Golf La Huerta</div>
      <button onClick={()=>setMode('pin')} style={{width:'100%',padding:16,border:'none',borderRadius:14,background:`linear-gradient(135deg,${D.gold},${D.goldLight})`,color:'#0A0A0A',fontSize:16,fontWeight:700,cursor:'pointer'}}>🏌️ Entrar como Admin</button>
      <button onClick={()=>setMode('spectator-input')} style={{width:'100%',padding:14,border:`1px solid ${D.border}`,borderRadius:14,background:'transparent',color:D.textSub,fontSize:15,fontWeight:600,cursor:'pointer'}}>👀 Ver ronda en vivo</button>
    </div>
  );
 
  /* ── PIN ── */
  if (mode === 'pin') return (
    <div style={{...app,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:24,gap:14}}>
      <div style={{fontSize:40,fontWeight:900,background:`linear-gradient(135deg,${D.gold},${D.goldLight})`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>H19</div>
      ...
      <input type="password" value={pinInput} onChange={e=>setPinInput(e.target.value)} placeholder="PIN" maxLength={6}
        style={{width:'100%',padding:'14px',border:`1px solid ${pinError?D.danger:D.border}`,borderRadius:12,background:D.surface,color:D.text,fontSize:22,textAlign:'center',letterSpacing:8,fontWeight:700}}/>
      {pinError && <div style={{color:D.danger,fontSize:13}}>PIN incorrecto</div>}
      <button onClick={()=>{ if(pinInput===ADMIN_PIN){setMode('admin');setPinError(false);} else{setPinError(true);} }}
        style={{width:'100%',padding:14,border:'none',borderRadius:12,background:`linear-gradient(135deg,${D.gold},${D.goldLight})`,color:'#0A0A0A',fontSize:15,fontWeight:700,cursor:'pointer'}}>Entrar</button>
  ...
  /* ── SPECTATOR INPUT ── */
  if (mode === 'spectator-input') return (
    <div style={{...app,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:24,gap:14}}>
      <div style={{fontSize:40,fontWeight:900,background:`linear-gradient(135deg,${D.gold},${D.goldLight})`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>H19</div>
      <div style={{fontSize:14,color:D.textSub,marginBottom:8,textAlign:'center'}}>Ingresa el código de ronda que te compartió el admin</div>
      <input value={spectatorInput} onChange={e=>setSpectatorInput(e.target.value.toUpperCase())} placeholder="Código de ronda" maxLength={8}
        style={{width:'100%',padding:'14px',border:`1px solid ${D.border}`,borderRadius:12,background:D.surface,color:D.text,fontSize:20,textAlign:'center',letterSpacing:4,fontWeight:700}}/>
      <button onClick={()=>{ if(spectatorInput.trim()){setRondaId(spectatorInput.trim());setMode('spectator');} }}
        style={{width:'100%',padding:14,border:'none',borderRadius:12,background:`linear-gradient(135deg,${D.gold},${D.goldLight})`,color:'#0A0A0A',fontSize:15,fontWeight:700,cursor:'pointer'}}>Ver ronda</button>
...
/* ═══════════════════════════════════════════════════
   ADMIN APP — toda la lógica de gestión
═══════════════════════════════════════════════════ */
function AdminApp({ onExit }) {
  ...
  const saveDir = (newPlayers, newNid) => {
    set(ref(db,'directorio'), { players: newPlayers, nextId: newNid||nid });
  };
 
  // Sync ronda to Firebase whenever game state changes
  const syncRonda = (state) => {
    if (!rondaId) return;
    set(ref(db, `rondas/${rondaId}`), { ...state, updatedAt: Date.now() });
  };
 
  const app = { fontSize:14, fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif', color:D.text, background:D.bg, minHeight:'100vh', maxWidth:420, margin:'0 auto', paddingBottom:32 };
  ...
  const startGame = () => {
    ...
    const campoData=CAMPOS[campo];
    const basePares=campoData.pares||Array(18).fill(4);
    ...
    // Generate short ronda ID
    const rid = Math.random().toString(36).substring(2,8).toUpperCase();
    setRondaId(rid);
    setPlayers(ps); setPars(p);
    setScores(initScores);
    setMarcas(initMarcas);
    setTarjetas(initTarjetas);
    setHole(0); setTab('score'); setResults(null); setScreen('score');
 
    // Save to Firebase
    set(ref(db, `rondas/${rid}`), {
      players:ps, pars:p, scores:initScores,
      marcas:initMarcas, tarjetas:initTarjetas,
      hole:0, campo, status:'en_juego', createdAt:Date.now(), updatedAt:Date.now()
    });
  };
 
  const updateGameState = (newState) => {
    if (!rondaId) return;
    set(ref(db, `rondas/${rondaId}`), { ...newState, updatedAt:Date.now() });
  };
 
  const getGameState = () => ({ players, pars, scores, marcas, tarjetas, hole, campo, status:'en_juego' });
 
  const commitHole = (sc,h) => sc.map(row=>row.map((v,j)=>j===h&&v===null?pars[j]:v));
 
  const changeScore = (pi,d) => {
    const newScores=scores.map((row,i)=>{ if(i!==pi) return row; return row.map((v,j)=>{ if(j!==hole) return v; return Math.max(1,(v===null?pars[j]:v)+d); }); });
    const s=newScores[pi][hole], p=pars[hole];
    const newMarcas=marcas.map((m,h)=>{ if(h!==hole) return m; const diff=s-p; const bogey=diff>=1; return {...m,multi:m.multi.map((row,i)=>{ if(i!==pi) return row; let nr={...row}; if(p===3){nr.holeinone=s===1;nr.eagle=false;nr.birdie=s===2;} else if(p===4){nr.holeinone=false;nr.eagle=s===2;nr.birdie=s===3;} else if(p===5){nr.holeinone=false;nr.eagle=s===3;nr.birdie=s===4;} if(bogey){nr.sandy=false;nr.holeout=false;} return nr; })}; });
    setScores(newScores); setMarcas(newMarcas);
    updateGameState({...getGameState(), scores:newScores, marcas:newMarcas});
  };
 
  const toggleMultiMarca = (pi,key) => {
    const newMarcas=marcas.map((m,h)=>{ if(h!==hole) return m; return {...m,multi:m.multi.map((row,i)=>i===pi?{...row,[key]:!row[key]}:row)}; });
    setMarcas(newMarcas);
    updateGameState({...getGameState(), marcas:newMarcas});
  };
  ...
  const assignTarjeta = (tkey,pi) => {
    ...
    updateGameState({...getGameState(), tarjetas:newTarjetas, marcas:newMarcas});
  };
 
  const nextHole = () => {
    const sc=commitHole(scores,hole); setScores(sc);
    const newHole=hole<nHoles-1?hole+1:hole;
    setHole(newHole); setTab('score');
    updateGameState({...getGameState(), scores:sc, hole:newHole});
  };
 
  const prevHole = () => {
    if(hole>0){ setHole(hole-1); setTab('score'); updateGameState({...getGameState(),hole:hole-1}); }
  };
 
  const finish = () => {
    const sc=commitHole(scores,hole); setScores(sc);
    const fullScores=sc.map(row=>row.map((v,j)=>v===null?pars[j]:v));
    const r=calcMoney(players,fullScores);
    const hc=calcHC(players,fullScores);
    const marcasMoney=calcMarcasMoney(players,marcas);
    const marcasPts=calcMarcasPts(players,marcas);
    const tarjetasMoney=calcTarjetasMoney(players,tarjetas);
    ...
    updateGameState({...getGameState(),scores:sc,status:'finalizada'});
    setScreen('res');
  };
  ...
  const confirmHC = () => {
    ...
    saveDir(updated);
    setSel(new Set()); setScreen('sel');
  };
 
  const shareRonda = () => {
    const url=`${window.location.origin}${window.location.pathname}?ronda=${rondaId}`;
    if(navigator.share){ navigator.share({title:'H19 - Ronda en vivo',url}); }
    else if(navigator.clipboard){ navigator.clipboard.writeText(url); setShareMsg('¡Link copiado!'); setTimeout(()=>setShareMsg(''),2500); }
  ...
  function marcasPtsHole(pi){ if(!marcas[hole]) return 0; let pts=0; MARCAS_MULTI.forEach(m=>{if(marcas[hole].multi[pi][m.key])pts+=m.pts;}); if(marcas[hole].oyes===pi)pts+=1; if(marcas[hole].regulation===pi)pts+=1; return pts; }
    ...
    <div style={app}>
      <div style={{background:D.surface,borderBottom:`1px solid ${D.border}`,padding:'20px 16px 14px'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div style={{fontSize:28,fontWeight:900,background:`linear-gradient(135deg,${D.gold},${D.goldLight})`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>H19</div>
        ...
        <div style={{fontSize:11,color:D.textSub,letterSpacing:2,textTransform:'uppercase',marginTop:2}}>Admin · Club de Golf La Huerta</div>
      </div>
        ...
        <Card>
          ...
          {dir.map((p,idx)=>(
            <div key={p.id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 0',borderBottom:idx<dir.length-1?`1px solid ${D.border}`:'none'}}>
                ...
                {editingHC===p.id?(
                  <div style={{display:'flex',alignItems:'center',gap:6,marginTop:4}}>
                    ...
                    <input type="number" min="0" max="54" defaultValue={p.hc} autoFocus
                      ...
                      onKeyDown={e=>{ if(e.key==='Enter'){const v=Math.max(0,parseInt(e.target.value)||0);saveDir(dir.map(d=>d.id===p.id?{...d,hc:v}:d));setEditingHC(null);} if(e.key==='Escape')setEditingHC(null); }}
              ...
              <button onClick={()=>setEditingHC(editingHC===p.id?null:p.id)} style={{padding:'5px 10px',border:`1px solid ${editingHC===p.id?D.gold:D.border}`,borderRadius:8,background:editingHC===p.id?D.goldDim:'transparent',color:editingHC===p.id?D.gold:D.textSub,fontSize:11,cursor:'pointer'}}>{editingHC===p.id?'✓ Listo':'Editar HC'}</button>
              {confirmDelete===p.id?(
    ...
    <div style={app}>
      ...
      <div style={{padding:'12px 12px'}}>
        ...
        <Card>
          <SLabel>Hoyos</SLabel>
          <div style={{display:'flex',gap:8}}>
            {[9,18].map(h=>{ const bp=CAMPOS[campo].pares||Array(18).fill(4); const pt=bp.slice(0,h).reduce((a,b)=>a+b,0); return <button key={h} onClick={()=>setNHoles(h)} style={tog(nHoles===h)}>{h} hoyos · Par {pt}</button>; })}
          </div>
        </Card>
        <Card>
          ...
          {n>=2&&<div style={{fontSize:12,color:D.textSub,padding:'8px 12px',background:D.surface,borderRadius:10,marginBottom:12,border:`1px solid ${D.border}`}}>🏆 Premio <span style={{color:D.gold,fontWeight:700}}>${pot}</span> · {n>=10?`1er $${Math.round(pot*0.6)} (60%) · 2do $${Math.round(pot*0.4)} (40%)`:'Todo para el 1er lugar'}</div>}
          {dir.map((p,idx)=>(
            <div key={p.id} onClick={()=>toggleSel(p.id)} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 0',borderBottom:idx<dir.length-1?`1px solid ${D.border}`:'none',cursor:'pointer',userSelect:'none'}}>
              <div style={{width:22,height:22,borderRadius:6,border:`2px solid ${sel.has(p.id)?D.gold:D.border}`,background:sel.has(p.id)?D.goldDim:'transparent',color:D.gold,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,flexShrink:0}}>{sel.has(p.id)?'✓':''}</div>
              <Avatar name={p.name} id={p.id} size={32}/>
  ...
  if(screen==='score') return (
    <div style={app}>
      <div style={{height:3,background:D.border}}><div style={{height:'100%',width:`${(hole/nHoles)*100}%`,background:`linear-gradient(90deg,${D.gold},${D.goldLight})`,transition:'width 0.3s'}}/></div>
      ...
      <div style={{padding:'0 12px'}}>
        ...
        {/* SCORE */}
        {tab==='score'&&<Card>{players.map((pl,i)=>{ const disp=getDisplay(i,hole),b=BADGE(disp,par); return(
          <div key={pl.id} style={{display:'flex',alignItems:'center',gap:10,padding:'11px 0',borderBottom:i<players.length-1?`1px solid ${D.border}`:'none'}}>
            <Avatar name={pl.name} id={pl.id} size={30}/>
            <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600}}>{pl.name}</div><div style={{fontSize:11,color:D.textSub}}>HC {pl.hc}</div></div>
        ...
        {tab==='marcas'&&marcas[hole]&&<div>
          ...
          <Card>
            ...
            {pars[hole]===3&&<div style={{marginBottom:16}}>
        ...
        {tab==='tarjetas'&&<Card>
          ...
          {TARJETAS.map((tj,idx)=>{ const owner=tarjetas[tj.key]; return(
            <div key={tj.key} style={{marginBottom:14,paddingBottom:14,borderBottom:idx<TARJETAS.length-1?`1px solid ${D.border}`:'none'}}>
              ...
              <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
                {players.map((pl,pi)=><div key={pl.id} onClick={()=>assignTarjeta(tj.key,pi)} style={{display:'flex',alignItems:'center',gap:6,padding:'6px 12px',border:`1px solid ${owner===pi?D.danger:D.border}`,borderRadius:20,background:owner===pi?D.redBg:'transparent',color:owner===pi?D.danger:D.textSub,fontSize:12,fontWeight:600,cursor:'pointer',userSelect:'none'}}><Avatar name={pl.name} id={pl.id} size={18}/>{pl.name}</div>)}
          ...
          <div style={{marginTop:8,padding:'10px 12px',background:D.surface,borderRadius:10}}>
            ...
            {players.map((pl,pi)=>{ const mc=TARJETAS.filter(t=>tarjetas[t.key]===pi); return(
        ...
        {/* TABLA */}
        {tab==='tabla'&&<div style={{overflowX:'auto',marginBottom:12}}>
          <table style={{borderCollapse:'collapse',width:'100%',fontSize:11,color:D.text}}>
            <thead>
              <tr style={{background:D.surface}}>
            ...
            <tbody>
              {players.map((pl,pi)=>{ const total=scores[pi].reduce((a,b)=>a+(b||0),0); const net=total-pl.hc; return(
                <tr key={pl.id} style={{borderBottom:`1px solid ${D.border}`}}>
                  <td style={{padding:'8px 10px',position:'sticky',left:0,background:D.card,zIndex:1}}>
                    <div style={{display:'flex',alignItems:'center',gap:6}}><Avatar name={pl.name} id={pl.id} size={22}/><span style={{fontSize:11,fontWeight:600,whiteSpace:'nowrap'}}>{pl.name}</span></div>
                  </td>
                  {pars.map((par,hi)=>{ const s=scores[pi][hi]; if(s===null||s===undefined) return <td key={hi} style={{textAlign:'center',padding:'6px 4px',color:D.textDim}}>·</td>; const d=s-par; let cb='transparent',cc=D.text,cbr='transparent',r=2; if(d<=-2){cb='#1A2744';cc='#7BAEE8';cbr='#7BAEE8';r='50%';} else if(d===-1){cb='#1A3020';cc='#7EC68A';cbr='#7EC68A';r=4;} else if(d===1){cc='#C8904A';cbr='#C8904A';} else if(d>=2){cc=D.danger;cbr=D.danger;} return <td key={hi} style={{textAlign:'center',padding:'4px 2px'}}><div style={{width:24,height:24,borderRadius:r,border:`1.5px solid ${cbr}`,background:cb,color:cc,display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,margin:'0 auto'}}>{s}</div></td>; })}
        ...
        {rondaId&&<div style={{textAlign:'center',marginBottom:8}}>
  ...
  if(screen==='res'&&results) {
    ...
    const ranked=players.map((p,i)=>({...p,net:nets[i],scoreMoney:money[i],marcasMoney:marcasMoney[i],tarjetasMoney:tarjetasMoney[i],total:money[i]+marcasMoney[i]+tarjetasMoney[i],pts:marcasPts[i],cards:tarjetasCount[i],bruto:fullScores[i].reduce((a,b)=>a+b,0)})).sort((a,b)=>a.net-b.net);
    const fn=fi.map(i=>players[i].name).join(' · '), fp=money[fi[0]], nn=players.length;
    return (
      <div style={app}>
        <div style={{background:`linear-gradient(135deg,#1A1200,#2A2000)`,borderBottom:`1px solid ${D.gold}44`,padding:'28px 16px',textAlign:'center',marginBottom:16}}>
          <div style={{fontSize:36,marginBottom:8}}>🏆</div>
          <div style={{fontSize:11,color:D.gold,letterSpacing:2,textTransform:'uppercase',marginBottom:6}}>1er Lugar · Score</div>
          <div style={{fontSize:24,fontWeight:900,marginBottom:4}}>{fn}</div>
          <div style={{fontSize:13,color:D.textSub,marginBottom:12}}>{nets[fi[0]]} golpes netos{fi.length>1?' · Premio dividido':''}</div>
          <div style={{fontSize:32,fontWeight:900,background:`linear-gradient(135deg,${D.gold},${D.goldLight})`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>+${fp}.00{fi.length>1?' c/u':''}</div>
        </div>
        {nn>=10&&fi.length===1&&si.length>0&&<div style={{background:D.surface,border:`1px solid ${D.border}`,margin:'0 12px 12px',borderRadius:16,padding:20,textAlign:'center'}}>
        ...
        <div style={{padding:'0 12px'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
            ...
            <button onClick={()=>setShowTabla(s=>!s)} style={{padding:'5px 12px',border:`1px solid ${showTabla?D.gold:D.border}`,borderRadius:20,background:showTabla?D.goldDim:'transparent',color:showTabla?D.gold:D.textSub,fontSize:11,fontWeight:700,cursor:'pointer'}}>{showTabla?'📋 Tabla':'📊 Desglose'}</button>
          </div>
 
          {/* Tabla resultados */}
          {showTabla&&<div style={{overflowX:'auto',marginBottom:12}}>
            <table style={{borderCollapse:'collapse',width:'100%',fontSize:11,color:D.text}}>
              <thead>
                <tr style={{background:D.surface}}>
                  ...
                  {results.fullScores[0].map((_,hi)=><td key={hi} style={{padding:'6px 4px',textAlign:'center',fontWeight:700,color:D.textSub,borderBottom:`1px solid ${D.border}`,minWidth:28,fontSize:10}}>{hi+1}</td>)}
                  ...
                  <td style={{padding:'6px 4px',textAlign:'center',fontWeight:700,color:'#7EC68A',borderBottom:`1px solid ${D.border}`,minWidth:34,borderLeft:`1px solid ${D.border}`}}>MK pts</td>
                  <td style={{padding:'6px 4px',textAlign:'center',fontWeight:700,color:'#7EC68A',borderBottom:`1px solid ${D.border}`,minWidth:44}}>MK $</td>
              ...
              <tbody>
                {ranked.map((p,pos)=>{ const pi=players.findIndex(pl=>pl.id===p.id); return(
                  <tr key={p.id} style={{borderBottom:`1px solid ${D.border}`,background:pos===0?'#1A1200':D.card}}>
                    <td style={{padding:'8px 10px',position:'sticky',left:0,background:pos===0?'#1A1200':D.card,zIndex:1}}>
                      <div style={{display:'flex',alignItems:'center',gap:5}}><span style={{fontSize:12,fontWeight:700,color:pos===0?D.gold:D.textSub}}>{pos+1}</span><Avatar name={p.name} id={p.id} size={20}/><span style={{fontSize:10,fontWeight:600,whiteSpace:'nowrap'}}>{p.name}</span></div>
                    </td>
                    {results.fullScores[pi].map((s,hi)=>{ const par2=pars[hi],d=s-par2; let cb='transparent',cc=D.text,cbr='transparent',r=2; if(d<=-2){cb='#1A2744';cc='#7BAEE8';cbr='#7BAEE8';r='50%';} else if(d===-1){cb='#1A3020';cc='#7EC68A';cbr='#7EC68A';r=4;} else if(d===1){cc='#C8904A';cbr='#C8904A';} else if(d>=2){cc=D.danger;cbr=D.danger;} return <td key={hi} style={{textAlign:'center',padding:'4px 2px'}}><div style={{width:22,height:22,borderRadius:r,border:`1.5px solid ${cbr}`,background:cb,color:cc,display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,fontWeight:700,margin:'0 auto'}}>{s}</div></td>; })}
                    ...
                    <td style={{textAlign:'center',padding:'6px 4px',fontWeight:700,fontSize:11,color:'#7EC68A',borderLeft:`1px solid ${D.border}`}}>{p.pts}</td>
          ...
          {!showTabla&&ranked.map((p,pos)=>(
            <Card key={p.id} style={{borderColor:pos===0?`${D.gold}66`:D.border}}>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12,paddingBottom:12,borderBottom:`1px solid ${D.border}`}}>
                ...
                <Avatar name={p.name} id={p.id} size={36}/>
                <div style={{flex:1}}><div style={{fontSize:15,fontWeight:700}}>{p.name}</div><div style={{fontSize:11,color:D.textSub}}>HC {p.hc} · {p.net} golpes netos</div></div>
                <div style={{textAlign:'right'}}><div style={{fontSize:20,fontWeight:900,color:fmtC(p.total)}}>{fmt(p.total)}</div><div style={{fontSize:10,color:D.textSub,textTransform:'uppercase',letterSpacing:1}}>Total</div></div>
              </div>
              {[{icon:'📊',label:'Score',sub:`${p.bruto} bruto · HC ${p.hc} · ${p.net} neto`,val:p.scoreMoney},{icon:'⭐',label:'Marcas',sub:`${p.pts} puntos · $10 por punto`,val:p.marcasMoney},{icon:'🃏',label:'Tarjetas',sub:`${p.cards} tarjeta${p.cards!==1?'s':''} · $10 por tarjeta`,val:p.tarjetasMoney}].map((row,i,arr)=>(
                <div key={row.label} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:i<arr.length-1?`1px solid ${D.border}`:'none'}}>
                  <div><div style={{fontSize:12,fontWeight:700,marginBottom:2}}>{row.icon} {row.label}</div><div style={{fontSize:11,color:D.textSub}}>{row.sub}</div></div>
              ...
              const myMarcas=[];
              marcas.forEach((holeData,hi)=>{ MARCAS_MULTI.forEach(m=>{if(holeData.multi[pi][m.key])myMarcas.push({hoyo:hi+1,label:m.label,pts:m.pts});}); if(holeData.oyes===pi)myMarcas.push({hoyo:hi+1,label:"📍 O'Yes",pts:1}); if(holeData.regulation===pi)myMarcas.push({hoyo:hi+1,label:'🎯 Regulation',pts:1}); });
              return(
                <div key={pl.id} style={{padding:'10px 0',borderBottom:pi<players.length-1?`1px solid ${D.border}`:'none'}}>
                  ...
                  {myMarcas.length>0?<div style={{paddingLeft:36,display:'flex',flexWrap:'wrap',gap:6}}>{myMarcas.map((m,i)=><div key={i} style={{display:'flex',alignItems:'center',gap:4,padding:'3px 8px',background:D.surface,border:`1px solid ${D.border}`,borderRadius:12,fontSize:11}}><span>{m.label}</span><span style={{color:D.textSub}}>H{m.hoyo}</span><span style={{color:D.gold,fontWeight:700}}>+{m.pts}pt</span></div>)}</div>:<div style={{paddingLeft:36,fontSize:11,color:D.textDim}}>Sin marcas</div>}
                </div>
              );
            })}
          </Card>
          ...
          <Card>
            ...
            {TARJETAS.map((tj,idx)=>{ const owner=tarjetas[tj.key]; return(
              <div key={tj.key} style={{display:'flex',alignItems:'center',gap:8,padding:'7px 0',borderBottom:idx<TARJETAS.length-1?`1px solid ${D.border}`:'none'}}>
                ...
                {owner!==null?<><Avatar name={players[owner].name} id={players[owner].id} size={22}/><div style={{fontSize:13,fontWeight:700,color:D.danger,marginLeft:4}}>{players[owner].name}</div></>:<div style={{fontSize:12,color:D.textDim}}>Sin dueño</div>}
              </div>
            );})}
          </Card>
          ...
          <Card>
            ...
            {hcUpdates.map((u,i)=>{ const up=u.delta>0,dn=u.delta<0; return(
              <div key={u.id} style={{display:'flex',alignItems:'center',gap:8,padding:'9px 0',borderBottom:i<hcUpdates.length-1?`1px solid ${D.border}`:'none'}}>
                ...
                <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600}}>{u.name}</div><div style={{fontSize:11,color:D.textSub}}>HC {u.before}</div></div>
