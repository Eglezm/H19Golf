...
function ScoreCell({ s, par, size = 24 }) {
  ...
  const d = s - par;
  const fs = size * 0.46;
  const base = { width:size, height:size, display:"flex", alignItems:"center", justifyContent:"center", fontSize:fs, fontWeight:700, margin:"0 auto", position:"relative", boxSizing:"border-box" };

  if (d <= -2) {
    // Eagle o mejor: doble círculo dorado
    return (
      <div style={{ ...base, borderRadius:"50%", border:`2px solid ${D.gold}`, outline:`2px solid ${D.gold}`, outlineOffset:"2px", color:D.gold, background:"#FDF3D0" }}>{s}</div>
    );
  }
  if (d === -1) {
    // Birdie: círculo verde
    return (
      <div style={{ ...base, borderRadius:"50%", border:`2px solid #1A5C24`, color:"#1A5C24", background:"#D4EDD8" }}>{s}</div>
    );
  }
  if (d === 0) {
  ...
  if (d === 1) {
    // Bogey: cuadro naranja
    return (
      <div style={{ ...base, border:`2px solid #C87A30`, color:"#8A4A00", background:"#FFF0D4" }}>{s}</div>
    );
  }
  if (d === 2) {
    // Doble bogey: doble cuadro rojo
    return (
      <div style={{ ...base, border:`2px solid #C62828`, outline:`2px solid #C62828`, outlineOffset:"2px", color:"#C62828", background:"#FFEBEB" }}>{s}</div>
    );
  }
  // Triple bogey o peor: doble cuadro rojo oscuro
  return (
    <div style={{ ...base, border:`2px solid #7B0000`, outline:`2px solid #7B0000`, outlineOffset:"2px", color:"#7B0000", background:"#FFD5D5", fontWeight:900 }}>{s}</div>
  );
}
                    ...
                    return (
                      <tr key={p.id} style={{ borderTop:`1px solid ${D.border}` }}>
                        ...
                        {pars.map((par, h) => {
                          const s = row[h];
                          return (
                            <td key={h} style={{ textAlign:"center", padding:"3px 1px", background:h===(hole||0)?D.goldDim+"55":"transparent" }}>
              ...
              <table style={{ borderCollapse:"collapse", width:"100%", fontSize:11, color:D.text }}>
                <thead>
                  <tr style={{ background:D.surface }}>
                    ...
                    {fullScores[0].map((_,hi) => <td key={hi} style={{ padding:"6px 3px", textAlign:"center", fontWeight:700, color:D.textSub, borderBottom:`1px solid ${D.border}`, minWidth:24, fontSize:10 }}>{hi+1}</td>)}
                ...
                <tbody>
                  {resTabla.map((p, pos) => (
                    <tr key={p.id} style={{ borderBottom:`1px solid ${D.border}`, background:pos===0?D.goldDim:D.card }}>
                      ...
                      {fullScores[p.pi].map((s,hi) => (
                        <td key={hi} style={{ textAlign:"center", padding:"3px 1px" }}><ScoreCell s={s} par={pars[hi]} size={20} /></td>
                      ))}
